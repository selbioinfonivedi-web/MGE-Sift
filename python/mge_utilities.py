...
class ShellPipelineStage(PipelineStage):
    ...
    def validate_output(self, output_dir: Path) -> bool:
        ...
        if expected_files:
            self.logger.info(f"Validation passed: found {len(expected_files)} output files")
            return True
        else:
            self.logger.warning(f"Validation warning: no output files found for {self.name}")
            return False
        ...
        for sample in samples:
            ...
            for stage_name, stage in self.stages.items():
                ...
                if not stage.execute(sample):
                    self.logger.error(f"Stage {stage_name} failed for {sample.sample_id}")
                    all_passed = False
                    continue
